    ...
    run: (template, context) => {
      for (const entry of template) {
        ...
        if (_token.name === '#if' || _token.name === '#elseif') {
          const _template = `{{ ${_token.expression} }}`
          // Parse expression
          const _parsed = TRANSFORM.parse(context, _template)
          // If 
          if (_parsed === _template) {
            return template
          } else {
            if (_parsed) {
              return TRANSFORM.run(entry[_key], context)
            }
          }
        } else {
          // Process `#else`
 ...
 */
  const $operators = {
    ...
    '#?': (options = {}) => {
      ...
      if (_result && _result !== _template) { return _result }
 ...
 */
    string: (template, context) => {
      ...
      if (!_reInclude.test(template)) {
        const _parsed = TRANSFORM.parse(context, template)
        return _parsed
      }
      ...
      return _token.expression ?
        TRANSFORM.parse(context, '{{' + _token.expression + '}}', true) :
        template
    },
 ...
 */
    array: (template, context) => {
      return Conditional.is(template) ?
        Conditional.run(template, context) :
        template.reduce((acc, entry) => {
          const _transform = TRANSFORM.run(entry, context)
 ...
 */
    object: (template, context) => {
      ...
      if (_matchingEntries.length > 0) {
        ...
        const _processed = _handler && _handler({ template, data: context, entry: _entry, token: _token })
        _result = _processed
      }
      ...
      for (const entry in template) {
        ...
        if (_isTemplate) {
          ...
          if (_token) {
            if (_token.name !== '#include') {
              const _handler = $operators[_token?.name]
              const _processed = _handler && _handler({ template, data: context, entry: entry, token: _token })
              if (_processed) {
                _result = _token.name === '#merge' ? { ..._result, ..._processed } : _processed
              }
            }
          } else {
            const _entry = TRANSFORM.parse(context, entry)
            const _value = TRANSFORM.parse(context, template[entry])
        ...
        if (!_isTemplate && _isString) {
          ...
          const _handler = $operators[_token?.name]
          if (_handler) {
            const _processed = _handler({ template, data: context, entry: entry, token: _token })
            if (_processed) {
              _result[entry] = _processed
            }
          } else {
            const _processed = TRANSFORM.run(template[entry], context)
 ...
 */
    parse: (context, template, raw) => {
      ...
      if (raw) {
        ...
        _template = TRANSFORM._parse({ variable: _variable, data: context, template: null })
      } else {
        _variables.forEach(_variable => {
          _template = TRANSFORM._parse({ variable: _variable, data: context, template: _template })
        })
      }
 ...
 */
    _parse: (options) => {
      ...
      const _makeFunction = (fnBody, context) => {
      ...
      try {
        ...
        const _resolver = _hasFunction ?
          _makeFunction(_hasFunction[1], data) :
          _makeFunction(slot, data)
